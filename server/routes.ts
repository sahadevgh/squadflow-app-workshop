...
router.get('/learners/:id', (req: Request, res: Response) => {
  const learner = getLearnerById(req.params.id);
  if (!learner) {
...
router.get('/learners/:id/progress', (req: Request, res: Response) => {
  const progress = getLearnerProgress(req.params.id);
  if (!progress) {
    return res.status(404).json({ error: 'Learner not found' });
  }
  res.json(progress);
...
router.post('/learners', (req: Request, res: Response) => {
